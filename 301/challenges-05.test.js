...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < ing.length; i++) {
    ...
    switch (i) {
    case 0:
      newStr = ing[i].slice(15);
      result.push(newStr);
      break;
    case 1: case 2: case 3:
      newStr = ing[i].slice(9);
      result.push(newStr);
      break;
    case 4: case 9: case 10:
      newStr = ing[i].slice(10);
      result.push(newStr);
      break;
    case 5: case 6: case 7: case 8:
      newStr = ing[i].slice(8);
      result.push(newStr);
      break;
    default:
      result.push(ing[i]);
...
const splitFoods = (recipe) => {
  let result = [];
  let regex = /(^\d+\s\w+\s|^\d+\s\w+\W\w+\s)/g;
  let ing = recipe.ingredients;
  for (let i = 0; i < ing.length; i++) {
    let newStr = ing[i].split(regex);
...
const stepActions = (recipe) => {
  let result = [];
  let regex = /(^\w+-\w+|^\w+)/g;
  let objSteps = recipe.steps;
  for (let i = 0; i < recipe.steps.length; i++) {
    let newStr = objSteps[i].split(regex);
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters > 0) {
    let param = str.length - numberOfCharacters;
    let newStr = str.slice(0, param);
    return newStr;
  }
};
