...
const count = (target, input) => {
  let result = input.reduce((sum, arr) => {
...
let findMaleAndFemale = (data) => {
  let newData = data.filter(prop => {
    if (prop.gender === 'male' || prop.gender === 'female') {
      return true;
  ...
  let result = newData.reduce((accu, obj, idx) => {
